...
var ASM_CONSTS = {
  ...
  163204: () => (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined),
  163308: () => {
    try {
  ...
  167879: $0 => miniaudio.get_device_by_index($0).webaudio.sampleRate,
  167945: $0 => {
...
function __mmap_js(len, prot, flags, fd, offset_low, offset_high, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, len, prot, flags, fd, offset_low, offset_high, allocated, addr);
...
function __munmap_js(addr, len, prot, flags, fd, offset_low, offset_high) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0, 1, addr, len, prot, flags, fd, offset_low, offset_high);
  var offset = convertI32PairToI53Checked(offset_low, offset_high);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    if (prot & 2) {
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, target, deferUntilInEventHandler);
  target = findEventTarget(target);
  if (!target) return -4;
  if (!target.requestPointerLock) {
    return -1;
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, fd, offset_low, offset_high, whence, newOffset);
  var offset = convertI32PairToI53Checked(offset_low, offset_high);
  try {
    if (isNaN(offset)) return 61;
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function _emscripten_set_window_title(title) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(36, 0, 1, title);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_fcntl64, ___syscall_fstat64, ___syscall_getcwd, ___syscall_ioctl, ___syscall_lstat64, ___syscall_mkdirat, ___syscall_newfstatat, ___syscall_openat, ___syscall_readlinkat, ___syscall_stat64, __mmap_js, __munmap_js, _emscripten_exit_pointerlock, _emscripten_get_element_css_size, _emscripten_get_gamepad_status, _emscripten_get_num_gamepads, _emscripten_request_pointerlock, _emscripten_sample_gamepad_data, _emscripten_set_click_callback_on_thread, _emscripten_set_fullscreenchange_callback_on_thread, _emscripten_set_gamepadconnected_callback_on_thread, _emscripten_set_gamepaddisconnected_callback_on_thread, _emscripten_set_touchcancel_callback_on_thread, _emscripten_set_touchend_callback_on_thread, _emscripten_set_touchmove_callback_on_thread, _emscripten_set_touchstart_callback_on_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_fdstat_get, _fd_read, _fd_seek, _fd_write, _emscripten_set_window_title ];
...
var _getFileLockFromUrl = Module["_getFileLockFromUrl"] = createExportWrapper("getFileLockFromUrl", 2);

var _endDownload2 = Module["_endDownload2"] = createExportWrapper("endDownload2", 1);

var _endDownload = Module["_endDownload"] = createExportWrapper("endDownload", 1);

var _setMatchId = Module["_setMatchId"] = createExportWrapper("setMatchId", 1);

var _setMatchId2 = Module["_setMatchId2"] = createExportWrapper("setMatchId2", 1);

var _wssRecive = Module["_wssRecive"] = createExportWrapper("wssRecive", 1);

var _wssRecive2 = Module["_wssRecive2"] = createExportWrapper("wssRecive2", 1);
